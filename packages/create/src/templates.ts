...
/**
 * The notify mount. One source for `kyb init --studio`, `kyb upgrade` and the
 * registry item, so the three can never disagree about what the file says.
 */
export function notifyMountTs(): string {
  return `// Tell the person's phone when this agent parks on a question or finishes a
// reply while they are away. The person is the verified session principal;
// the control plane does the ringing. See @kybernesis/notify.
